...
support.
\new{It has been designed with database operators and optimization
techniques targeting a fast execution of these operators.
At the same time, it is a general high-level IR, and thus, can serve not only
for the optimization of standard SQL operators but also for new sets of
operators (e.g., developed for more complex data analysis),
new categories of built-in functions with pre-defined behaviours can be added
easily, and it could even be used by other array-based languages such as MATLAB
as an intermediate representation.
Thus, compared to query compilers in database systems such as HyPer, it is more
general and extensible as HyPer performs a direct translation from relational
algebra operators to optimized code.}
...
finally the optimized code is compiled into C code executable on different
platforms.
